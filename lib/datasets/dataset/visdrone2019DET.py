...
class VisDrone2019DET(data.Dataset):
    ...
    def _summarize_detection_counts(self, detections, score_thresh=0.3,
                                    iou_thresh=0.5):
        """Summarize serialized detections using class-aware IoU matching."""
        true_positives = 0
        false_positives = 0
        false_negatives = 0
        predicted_count = 0
        ...
        for cls_ind, class_name in enumerate(self.class_name, start=1):
            ...
            for score, image_id, bbox in predictions:
                del score  # Score determines matching order only.
                gt_boxes = []
            ...
            for image_id in self.images:
                ...
                class_fn += gt_count - len(matched_ground_truth.get(image_id, set()))
