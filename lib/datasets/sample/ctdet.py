...
class CTDetDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.opt.keep_res:
            ...
            s = max(sharp_img.shape[0], sharp_img.shape[1]) * 1.0
        ...
        if self.split == 'train':
            if not self.opt.not_rand_crop:
                ...
                w_border = self._get_border(128, sharp_img.shape[1])
                h_border = self._get_border(128, sharp_img.shape[0])
                c[0] = np.random.randint(low=w_border, high=sharp_img.shape[1] - w_border)
                c[1] = np.random.randint(low=h_border, high=sharp_img.shape[0] - h_border)
            else:
                sf = self.opt.scale
                cf = self.opt.shift
